    ...
    bcrypt.hash(req.body.password, 10) // La méthode bcrypt.hash() permet de hasher un mot de passe afin de le sécuriser. pour un hashage sécurisé, on le hash 10 fois, donc le salt = 10
        .then(hash => {
            ...
            user.save()
                ...
                .catch(error => {res.status(400).json({ error })}); // Une erreur 400, aussi appelée “400 Bad Request”, signifie que le serveur n’a pas pu comprendre ou traiter la requête envoyée par ton navigateur
        })
